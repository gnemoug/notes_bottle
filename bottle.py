...
#分清楚他们各自(DictProperty、CachedProperty)做装饰器的区别
...
#why use this?
#because this is to implement descriptor,for detail information, you can take a look at http://docs.python.org/2/reference/datamodel.html#implementing-descriptors
class CachedProperty(object):
    ...
    def __get__(self, obj, cls):
        ...
        if obj is None: return self#表示这是类的access
...
class HTTPResponse(BottleException):
    ...
    def __init__(self, output='', status=200, header=None):
        ...
        self.output = output
        self.headers = HeaderDict(header) if header else None#A case-insensitive version of :class:`MultiDict`
...
####################注意下面各种异常的继承关系，这样可以通过类名来明确语义，确定犯了什么错误################
...
class Router(object):
    #主要是为了通过正则来解析url，判断应该调用哪个Route对象
    ...
    def __init__(self, strict=False):
        self.rules    = {} # A {rule: Rule} mapping#保存所有路由
        self.builder  = {} # A rule/name->build_info mapping#保存解析每条路由的方法
        self.static   = {} # Cache for static routes: {path: {method: target}}#保存动态路由
        self.dynamic  = [] # Cache for dynamic routes. See _compile()#保存静态路由
        #: If true, static routes are no longer checked first.
        self.strict_order = strict#用于标志是否先检查静态路由
    ...
    def add(self, rule, method, target, name=None):
        ...
        #Fully identical routes (same path rule and method) replace previously defined routes, but keep the position of their predecessor.
            self.rules[rule][method] = target#同时在改变self.dynamic和self.static
        ...
        for key, mode, conf in self.parse_rule(rule):
            if mode:
                ...
                #mask为对正则中变量的匹配
                if key:
                    pattern += '(?P<%s>%s)' % (key, mask)#输出正则表达式，用于对url进行匹配
                else:
                    pattern += '(?:%s)' % mask
                    key = 'anon%d' % anons; anons += 1
                #目前in_filter中只存储key为int和float的
    ...
    def match(self, environ):
        ...
        if path in self.static:
            ...
            for combined, rules in self.dynamic:
                ...
                ####针对urlargs，若path为:'/fuck/you',则urlargs为:{}；若path为:'/hello/<name>/<age:int>',则urlargs为:{'age': 21, 'name': 'guomeng'}
                break
...
#value = obj.__dict__[self.func.__name__] = self.func(obj)
    @cached_property
    def call(self):
        #该函数只会执行一次，可以作为普通属性使用，返回要调用的最终函数
    ...
    def _context(self):
        ...
        #depr :发出DeprecationWarning,声明有的plugin是过期版本
        return dict(rule=self.rule, method=self.method, callback=self.callback,
                    name=self.name, app=self.app, config=self.config,
                    apply=self.plugins, skip=self.skiplist)

    def all_plugins(self):
        #产生所有对此route有影响的plugins
...
class Bottle(object):
    ...
    def __init__(self, catchall=True, autojson=True, config=None):
        ...
        self.router = Router(strict = True) # Maps requests to :class:`Route` instances.
    ...
    def mount(self, prefix, app, **options):
        ...
        def mountpoint():
            try:
                ...
                #改变:self['SCRIPT_NAME'], self['PATH_INFO']
                rs = BaseResponse([], 200)
    ...
    def route(self, path=None, method='GET', callback=None, name=None,
        ...
        """ A decorator to bind a function to a request URL. Example::
            ...
            :param path: Request path or a list of paths to listen to. If no
              ...
              signature of the function.      可以为一个list
            :param method: HTTP method (`GET`, `POST`, `PUT`, ...) or a list of
              methods to listen to. (default: `GET`),     可以为一个list
    ...
    def _handle(self, environ):
        ...
        try:
            ...
            return route.call(**args)#返回函数和插件解析返回结果
    ...
    def wsgi(self, environ, start_response):
        ...
        try:
            ...
            request.bind(environ)#初始化request对象，并重新设置request中数据
            response.bind()#重设response中内容
...
class BaseRequest(DictMixin):
    ...
    MEMFILE_MAX = 102400#用于标注每次最多读取多少内容进内存
    #: Maximum number pr GET or POST parameters per request
    MAX_PARAMS  = 100#最多请求的参数
...
#    print type(cookies)
    ...
    def json(self):
        ...
        and 0 < self.content_length < self.MEMFILE_MAX:
            ...
            #返回收到的json文件作为一个python对象
        return None

    @DictProperty('environ', 'bottle.request.body', read_only=True)
    def _body(self):
        """
            用于读取request的body内容，eg:请求的post内容或者上传的文件内容
        """
    ...
    def POST(self):
        ...
        #self.content_type：表示后面请求的文档属于什么MIME类型
        #multipart/form-data类型的POST实体结构相对来说（常规的POST正文采用application/x-www-form-urlencoded格式）比较复杂，它常用于文件上传。
        #用于正常返回请求的文档，eg:application/x-www-form-urlencoded
        #注意tonat函数，touni函数和tob函数的区别
...
#        {'c': 'weqre', 'b': 'daf'}
    ...
    @property
    def url(self):
        #获取完整的url ,eg：http://user:pwd@NetLoc:80/p1;param/p2;param?query=arg#frag
    ...
    def urlparts(self):
            #获取SplitResult类型的对象，eg:SplitResult(scheme='http', netloc='user:pwd@NetLoc:80', path='/p1;param/p2;param', query='query=arg', fragment='frag')
    ...
    def path_shift(self, shift=1):
        ...
        print self['SCRIPT_NAME'], self['PATH_INFO']
    ...
    def auth(self):
        ...
        #REMOTE_USER  如果提供了Authorization头，则代表其用户部分。它代表发出请求的用户的名字，用于更高级别的验证
...
class BaseResponse(object):
    ...
    def __init__(self, body='', status=None, **headers):
        ...
        self.status = status or self.default_status####相当于是在给self._status_code和self._status_line赋值
        if headers:
...
#def tob(data, enc='utf8'):
#    """ Convert anything to bytes """将unicode类型的dta转化为bytes类型的字节码文件
#    return data.encode(enc) if isinstance(data, unicode) else bytes(data)
...
#max_age
#是用秒表示的cookie最长生存期，一旦超过时间则就失效了。如果为0表示立即失效。而expire是用字符串表示的失效期。那么这两个参数其实是一样的，有什么区别呢？ 在RFC2109上说明，expire是Netscape最早定义的用来代替max-age的。
...
#>>> a.timetuple()
#time.struct_time(tm_year=2012, tm_mon=9, tm_mday=7, tm_hour=7, tm_min=38, tm_sec=19, tm_wday=4, tm_yday=251, tm_isdst=-1)
#>>> a = datetime.date.today()
#>>> a.timetuple()
#time.struct_time(tm_year=2012, tm_mon=9, tm_mday=7, tm_hour=0, tm_min=0, tm_sec=0, tm_wday=4, tm_yday=251, tm_isdst=-1)
...
#就是为了方便美观的获取对应的header中的属性值提供的简单封装，eg:a = WSGIHeaderDict(),a.get(content_length)
    ...
    def _ekey(self, key):
        ...
        if key in self.cgikeys:
            return key
        return 'HTTP_' + key#对应HTTP_ Variables:Variables corresponding to the client-supplied HTTP request headers
...
def _parse_qsl(qs):
    #将浏览器post的数据转化成（key,value）的list形式
    r = []
...
def cookie_encode(data, key):
    ...
    msg = base64.b64encode(pickle.dumps(data, -1))#the highest protocol version will be used.
...
def yieldroutes(func):
    ...
    #Get the names and default values of a Python function’s arguments. A tuple of four things is returned: (args, varargs, keywords, defaults).
...
def path_shift(script_name, path_info, shift=1):
    #用于挂载mount中，区分script_name和path_info
...
def make_default_app_wrapper(name):
    ...
    #wraps函数，它将update_wrapper也封装;update_wrapper函数，它可以把被封装函数的__name__、__module__、__doc__和 __dict__都复制到封装函数去
    @functools.wraps(getattr(Bottle, name))
    def wrapper(*a, **ka):
        return getattr(app(), name)(*a, **ka)
    return wrapper

#现在下面这些string都成为了可以作用于默认的Bottle对象上的函数，访问他们可以通过命名
...
#利用python 内嵌模块实现的非常简单的web server
...
def run(app=None, server='wsgiref', host='127.0.0.1', port=8080,
    ...
    if reloader and not os.environ.get('BOTTLE_CHILD'):
        try:
            #an OS-level handle to an open file,the absolute pathname of that file,将文件保存在/temp目录下.eg:/tmp/bottle.qQYaDr.lock
            fd, lockfile = tempfile.mkstemp(prefix='bottle.', suffix='.lock')
            os.close(fd) # We only need this file to exist. We never write to it
            while os.path.exists(lockfile):
                #sys.executable]:A string giving the absolute path of the executable binary for the Python interpreter,eg:/usr/bin/python
                args = [sys.executable] + sys.argv
                environ = os.environ.copy()
                environ['BOTTLE_CHILD'] = 'true'
                environ['BOTTLE_LOCKFILE'] = lockfile
                p = subprocess.Popen(args, env=environ)
                #p.poll():用于检查子进程是否已经结束。设置并返回returncode(获取进程的返回值)属性。
                while p.poll() is None: # Busy wait...如果进程还没有结束，返回None。
                    os.utime(lockfile, None) #os.utime: I am alive!#Set the access and modified times of the file specified by path.eg:linux 'touch'
                    time.sleep(interval)
                if p.poll() != 3:
                    #用于标志错误退出，eg:sys.exit(3) SyntaxError, ImportError
                    if os.path.exists(lockfile): os.unlink(lockfile)#Remove (delete) the file path. This is the same function as remove();
                    sys.exit(p.poll())
...
app = default_app = AppStack()#为一个保存bottle对象的堆栈
app.push()
